...
| `SPECS_DIR` | `specs/` のパス |
| `DOCS_DIR` | デザインリファレンスのパス (`stacks/nextjs/docs/`、段階 2 で stacks 配下に移動) |
...
### variant.py — プル子 / プタ子 の識別子

プロジェクトタイプ (プル子 = `nextjs` / プタ子 = `generic`) の印を `.claude/state/variant.json` に読み書きする。`install.py:_scan_skills` や将来の分岐ロジックが機械参照して挙動を切り替える (AI の自己判断は挟まない / `specs/08-responsibility.md`)。

| 関数 | 内容 |
|---|---|
| `read_variant()` | プロジェクトの variant を返す (本元では常に VARIANT_DEFAULT = nextjs) |
| `write_variant(value)` | variant.json に印を書き込む (本元では no-op、不正値は ValueError) |

後方互換: 印が無いプロジェクトは `VARIANT_DEFAULT` (= `nextjs`、既存プル子) にフォールバックする。`install.py --update` で印が無ければ自動的に `nextjs` を書き足す。
...
#### アニメーション取材・再現の組み込み (段階 4 追加 — 全 WebGL program 履歴追跡)

RSRC-WEBANIM-HARDCASE §1 の制約「CURRENT_PROGRAM 1 つのみ抽出」を解消し、サイトが描画ループ中に切替える全ての program を `spector.mjs` の `useProgram` hook で追跡する拡張:

- **useProgram hook**: `spector.mjs` に `installWebGLProgramTracking(page)` を新規追加。`WebGLRenderingContext.prototype.useProgram` / `WebGL2RenderingContext.prototype.useProgram` を `addInitScript` で patch し、`bind` された program を `WeakSet` で重複排除しながら `{gl, program, contextType}` のリストに蓄積する
- **recon.mjs の起動順**: page 生成直後 (仮想時計注入 / rrweb UMD 注入より前) に `installWebGLProgramTracking(page)` を呼ぶ。順序は「WebGL hook → 仮想時計 → rrweb UMD」
- **capture の分岐**: `captureWebGLShaders(page)` が `window.__webglTrackedPrograms__` の件数を見て、1 件以上あれば hook 経由 (全履歴)、無ければ CURRENT_PROGRAM フォールバック (段階 2 互換) で shader を抽出する
- **shaders.json 形式**: `programs[]` の 1 エントリあたり `{id, canvasIndex, contextType, vertexShader, fragmentShader}`。`contextType` は `webgl` / `webgl2` の区別、`id` は `program-<i>` (全履歴) または `canvas-<i>-current` (フォールバック)。既存の build 雛形 (`_build_tsx_skeleton_r3f`) は programs[0] を読むだけなので互換
- **制約**: program の GLSL は取材時点で `gl.getShaderSource` が返すソース (圧縮前)。program が `deleteProgram` で破棄済みの場合は try で握り捨てる。Worker 内の WebGL は hook されない

段階 5 (VRT 差分自動修正ループ) と段階 4 の残課題 (Vision LLM フォールバック、scroll-samples 自体の更なる速度改善) は別セッションで段階投入する。設計は `.libs/research/webanim/` の 3 ページに固定済み。
