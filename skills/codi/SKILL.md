...
- レイアウトタスクの場合: docs 検索は /layo が担当 (load.py ではスキップ)
...
ステップ 2 は `/layo` で策定済みの本棚ページを参照して実装する。

**前提:** `/layo $ARGUMENTS` でデザインが策定済みであること。
本棚ページが存在しない場合は「先に `/layo $ARGUMENTS` でデザインを策定してください」と案内して停止する。

```
.libs/storybook/<task-slug>/<task-slug>.md  ← /layo が作成した本棚ページ（正本）
```
