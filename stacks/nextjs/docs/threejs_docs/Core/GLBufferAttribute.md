...
> Source: https://threejs.org/docs/pages/GLBufferAttribute.html
> Category: Core
...
**buffer** |  The native WebGL buffer.  
---|---  
**type** |  The native data type (e.g. `gl.FLOAT`).  
**itemSize** |  The item size.  
**elementSize** |  The corresponding size (in bytes) for the given `type` parameter.  
...
### .setBuffer( buffer : WebGLBuffer ) : [BufferAttribute](BufferAttribute.html)
...
### .setCount( count : number ) : [BufferAttribute](BufferAttribute.html)
...
### .setItemSize( itemSize : number ) : [BufferAttribute](BufferAttribute.html)
...
### .setType( type : number, elementSize : number ) : [BufferAttribute](BufferAttribute.html)
