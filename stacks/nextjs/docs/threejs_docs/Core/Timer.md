...
> Source: https://threejs.org/docs/pages/Timer.html
> Category: Core
...
### .setTimescale( timescale : number ) : [Timer](Timer.html)
...
### .update( timestamp : number ) : [Timer](Timer.html)
...
**timestamp** |  The current time in milliseconds. Can be obtained from the `requestAnimationFrame` callback argument. If not provided, the current time will be determined with `performance.now`.  
---|---  
