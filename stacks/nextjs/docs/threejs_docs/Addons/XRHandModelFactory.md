...
> Source: https://threejs.org/docs/pages/XRHandModelFactory.html
> Category: Addons
...
### new XRHandModelFactory( gltfLoader : [GLTFLoader](GLTFLoader.html), onLoad : function )

Constructs a new XR hand model factory.

**gltfLoader** |  A glTF loader that is used to load hand models. Default is `null`.  
---|---  
...
### .createHandModel( controller : [Group](Group.html), profile : 'spheres' | 'boxes' | 'mesh' ) : [XRHandModel](XRHandModel.html)

Creates a controller model for the given WebXR hand controller.

**controller** |  The hand controller.  
---|---  
**profile** |  The model profile that defines the model type.  
  
**Returns:** The XR hand model.

### .setPath( path : string ) : [XRHandModelFactory](XRHandModelFactory.html)
