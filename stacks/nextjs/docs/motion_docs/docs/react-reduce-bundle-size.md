...
`[useAnimate](./react-use-animate)` is Motion for React's animation function, used for manually triggering and controlling animations.
...
The `[motion](./react-motion-component)`[ component](./react-motion-component) is Motion for React's most common animation API.

Because of its declarative, props-driven API, it's impossible for bundlers to tree shake it any smaller than **34kb**.

However, by using [the ](./react-lazy-motion)`[m](./react-lazy-motion)`[ and ](./react-lazy-motion)`[LazyMotion](./react-lazy-motion)`[ components](./react-lazy-motion), you can bring this down significantly, to just under **4.6kb** for the initial render.
...
`m` is used in the exact same way as `motion`, but unlike `motion`, the `m` component doesn't come preloaded with features like animations, [layout animations](./react-layout-animations), or the drag gesture.
