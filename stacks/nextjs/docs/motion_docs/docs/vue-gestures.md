...
`motion` components provide multiple gesture animation props: `whileHover`, `whilePress`, `whileFocus`, `whileDrag` and `[whileInView](./vue-scroll-animations)`. These can define animation targets to temporarily animate to while a gesture is active.
