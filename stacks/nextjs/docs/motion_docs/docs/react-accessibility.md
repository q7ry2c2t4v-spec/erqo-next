...
The `[reducedMotion](./react-motion-config)` option can be set on `MotionConfig` to define how you want to adhere to the Reduced Motion setting.

By setting `reducedMotion` it to `"user"`, all `motion` components will **automatically** disable transform and [layout animations](./react-layout-animations), while preserving the animation of other values like `opacity` and `backgroundColor`.
...
While `reducedMotion` is a great blanket tool for ensuring accessible animations across your whole site, more bespoke solutions can be created with [the ](./react-use-reduced-motion)`[useReducedMotion](./react-use-reduced-motion)`[ hook](./react-use-reduced-motion).
