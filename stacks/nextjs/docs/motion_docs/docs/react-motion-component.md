...
Building with AI? The [Motion Studio MCP](./studio-ai-context) gives your AI editor access to the latest docs and source code of 330+ examples.
...
By creating a `dragControls` using the `[useDragControls](./react-use-drag-controls)`[ hook](./react-use-drag-controls), we can pass this into the draggable component's `dragControls` prop. It exposes a `start` method that can start dragging from pointer events on other components.
