...
Check out the [Animation guide](./vue-animation) for a full overview on animations in Motion for Vue.
...
Using [motion values](./vue-motion-value) instead of Vue state to update `style` will also avoid re-renders.
...
**Note:** Owing to Vue Transition component limitations, the component being removed **must** be a **direct child** of `[AnimatePresence](./vue-animate-presence)` to enable this animation.
...
The [variants](./vue-animation#variants) for this component.
...
The normal Vue DOM `style` prop, with added support for [motion values](./vue-motion-value) and independent transforms. 
...
By creating a `dragControls` using the `[useDragControls](./vue-use-drag-controls)`[ hook](./vue-use-drag-controls), we can pass this into the draggable component's `dragControls` prop. It exposes a `start` method that can start dragging from pointer events on other components.
...
If `true`, this component will [animate changes to its layout](./vue-layout-animations).
