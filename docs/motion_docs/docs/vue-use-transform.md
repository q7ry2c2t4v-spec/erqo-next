...
The output range must be values all of the same type, but can be in any order. It can also be any [value type that Motion can animate](./vue-animation#animatable-values), like numbers, units, colors and other strings.
