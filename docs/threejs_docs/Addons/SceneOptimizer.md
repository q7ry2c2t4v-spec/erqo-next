...
> Source: https://threejs.org/docs/pages/SceneOptimizer.html
> Category: Addons
...
### new SceneOptimizer( scene : [Scene](Scene.html), options : [SceneOptimizer~Options](SceneOptimizer.html#~Options) )
...
### .disposeMeshes( meshesToRemove : Set.<[Mesh](Mesh.html)> )

Removes the given array of meshes from the scene.

**meshesToRemove** |  The meshes to remove.  
---|---  
  
### .removeEmptyNodes( object : [Object3D](Object3D.html) )

Removes empty nodes from all descendants of the given 3D object.

**object** |  The 3D object to process.  
---|---  
  
### .toBatchedMesh() : [Scene](Scene.html)
...
### .toInstancingMesh() : [Scene](Scene.html) (abstract) 
...
**debug**   
boolean |  Whether to enable debug mode or not. Default is `false`.  
---|---  
