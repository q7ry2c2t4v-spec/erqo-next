...
> Source: https://threejs.org/docs/pages/BokehPass.html
> Category: Addons
...
### new BokehPass( scene : [Scene](Scene.html), camera : [Camera](Camera.html), params : [BokehPass~Options](BokehPass.html#~Options) )

Constructs a new Bokeh pass.

**scene** |  The scene to render the DOF for.  
---|---  
**camera** |  The camera.  
**params** |  The pass options.  
...
### .materialBokeh : [ShaderMaterial](ShaderMaterial.html)
...
**focus**   
number |  Defines the effect's focus which is the distance along the camera's look direction in world units. Default is `1`.  
---|---  
**aperture**   
number |  Defines the effect's aperture. Default is `0.025`.  
**maxblur**   
number |  Defines the effect's maximum blur. Default is `1`.  
