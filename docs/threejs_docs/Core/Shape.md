...
[Curve](Curve.html) → [CurvePath](CurvePath.html) → [Path](Path.html) → 
...
### new Shape( points : Array.<[Vector2](Vector2.html)> )
...
### .holes : Array.<[Path](Path.html)> (readonly) 
...
### .getPointsHoles( divisions : number ) : Array.<Array.<[Vector2](Vector2.html)>>
