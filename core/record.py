...
def _build_replay_verification_section(task_id: str, bookshelf_path: Path) -> str | None:
    """baseline の差分計測結果 (diff-result.json) から features 用セクションを生成する。

    RSRC-WEBANIM-HARDCASE §7 "VRT 差分自動修正ループ" のレポート部分。
    diff-result.json が無い場合は None を返す (レイアウトでない or 未計測)。
    """
    diff_path = bookshelf_path.parent / DIFF_OUTPUT_DIR_NAME / DIFF_RESULT_FILENAME
    ...
    max_ratio = summary.get("maxMismatchRatio", DIFF_MAX_MISMATCH_RATIO)
    lines = [
        f"## {task_id}.再現度検証 — 再現度検証 (元サイト vs 再現)",
        "",
        "> baseline が pixelmatch で自動計測した差分率。"
        f"閾値 {max_ratio * 100:.1f}% 以下なら「OK」。"
        "超過時は RSRC-WEBANIM-HARDCASE §7 の差分ループで修正を検討する (段階 5)。",
        "",
        "| viewport | 差分率 | 判定 | diff 画像 |",
        "|---|---|---|---|",
    ]
    for r in results:
        vp = r.get("viewport", "?")
        ratio = float(r.get("mismatchRatio", 0.0))
        verdict = r.get("verdict", "?")
        diff_img = r.get("diff", "?")
        lines.append(
            f"| `{vp}` | {ratio * 100:.2f}% | {verdict} | `{diff_img}` |"
        )
    lines.append("")
    return "\n".join(lines)
...
def create_record(task_id: str) -> None:
    ...
    if _is_fix_task(task_id):
        ...
        # レイアウトタスクで diff-result.json があれば「再現度検証」セクションを追記
        if _is_layout_task(task_id) and bookshelf_path and bookshelf_path.exists():
            replay_section = _build_replay_verification_section(task_id, bookshelf_path)
            if replay_section:
