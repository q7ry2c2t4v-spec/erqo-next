...
タグ: アニメーション, WebGL, シェーダ, 逆解析, Spector, rrweb, 時間偽装, vision LLM, Lottie, Rive, pixelmatch, VRT, 差分ループ, hardcase, shader, reverse-engineer, time-virtualization
...
### 1. WebGL 完全取材 — Spector.js 統合

Babylon.js チームの `Spector.js` は WebGL / WebGL2 の 1 フレーム分を commands / programs / shaders / textures / uniforms まで全キャプチャする。**ブラウザ拡張ではなく standalone ライブラリ** として Playwright から `page.addScriptTag` で流し込める。

```js
// Playwright で recon.mjs 内から実行
await page.addScriptTag({ url: 'https://spectorcdn.babylonjs.com/spector.bundle.js' });
const capture = await page.evaluate(async () => {
  const spector = new window.SPECTOR.Spector();
  spector.spyCanvases();                    // 以降のすべてのWebGL呼び出しを監視
  const canvas = document.querySelector('canvas');
  return new Promise(resolve => {
    spector.onCapture.add(c => resolve(c)); // 1フレーム分のcapture
    spector.captureCanvas(canvas);
  });
});
// capture.programs[*].shaders[vertex|fragment].source に GLSL が入る
await fs.writeFile('shaders.json', JSON.stringify(capture, null, 2));
```

取れるもの:
- **programs[n].shaders.vertex.source** / **.fragment.source** — GLSL 原文 (ミニファイされていない状態)
- **commands** — `bindBuffer` / `uniformMatrix4fv` など全 WebGL 呼び出しの時系列
- **textures** / **uniforms** — どのテクスチャがどのスロットに、どの uniform にどの値が入っているか
- **attributes** / **VAOs** — ジオメトリ構造

これを `React Three Fiber` 側に移植するときは `shaderMaterial` の `vertexShader` / `fragmentShader` に GLSL をそのまま貼り、`uniforms` もキャプチャから移植する。**9 割のシェーダ系背景はここで完全再現できる。** 残りの 1 割 (WebGPU 移行済 / 独自バイナリ配信 / WebAssembly 駆動のグラフィクス) は別ルート。
...
| 手法 | 強み | 弱み | コスト |
|---|---|---|---|
| Spector.js | GLSL 完全抽出 | WebGPU 非対応、独自 binary 不可 | 低 (standalone 組込) |
...
- **WebGL ありサイト** → Spector.js + R3F 移植 + VRT ループ
...
1. **Spector.js を recon のオプトインサブステップに** — 本棚ページに `hasWebGL: true` が立ったら `recon-webgl` が走る形。`shaders.json` を `.libs/storybook/<slug>/webgl/` に保存。build 雛形が R3F + shaderMaterial を生成する。
